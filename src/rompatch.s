...
*****************************************
*	IOCS $7f	_ONTIME		*	(全ＲＯＭ)
*****************************************

* COR.氏の Hi-Speed ONTIME v1.10 より
...
.ifdef HONTIME
		move.l	($9d6),d0
		cmp.l	(hontime_work_3,pc),d0
		bcs	@f

		move.l	a0,d1		経過日数が増えた場合
		lea	(hontime_work_1,pc),a0
		divu	#60*24,d0
		move	d0,(a0)+	hontime_work_1
		mulu	#60*24,d0
		move.l	d0,(a0)+	hontime_work_2
		addi.l	#60*24,d0
		move.l	d0,(a0)		hontime_work_3
		movea.l	d1,a0

		move.l	($9d6),d0
@@
		sub.l	(hontime_work_2,pc),d0
		add	d0,d0
		add	d0,d0
		move.l	(hontime_table,pc,d0.w),d0
		moveq	#0,d1
		move	($9cc),d1
		sub.l	d1,d0
		move	(hontime_work_1,pc),d1
.else
		move.l	($9d6),d1
		cmpi.l	#60*24*65536,d1
		bcs	@f
		moveq	#0,d1
@@:
		divu	#24*60,d1
